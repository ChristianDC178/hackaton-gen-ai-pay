...
def call_bedrock_nova(prompt, system_prompt):
...
def get_benefits(province="Mendoza", name=""):
    print(f"### Requesting benefits for province: {province}")
    
    system_prompt = f"Eres un asistente de recomendaciones de promociones y sucursales. Datos disponibles: {json_data}"
    
    user_prompt = f"""Filtra las promociones en la provincia {province}. 
    Dame la salida en formato JSON válido con los campos: name, benefitValue, days, heading, locations.
    Agrega un campo message con un saludo personalizado para {name if name else 'el usuario'}."""
    
    try:
        response = call_bedrock_nova(user_prompt, system_prompt)
        return {
            'statusCode': 200,
            'headers': {
...
def lambda_handler(event, context):
    ...
    if event.get('queryStringParameters'):
        province = event['queryStringParameters'].get('province', 'Mendoza')
